...
const CodexPermissionReviewExplanation = Schema.String.check(
  Schema.isNonEmpty(),
...
export const CodexPermissionReviewDecision = Schema.Struct({
  ...
  risk_basis: CodexPermissionReviewExplanation,
  user_authorization: Schema.Literals(["unknown", "low", "medium", "high"]),
  authorization_basis: CodexPermissionReviewExplanation,
  decision: Schema.Literals(["allow", "deny", "ask_user"]),
  reason: CodexPermissionReviewExplanation,
});
...
- risk_basis must briefly name the risk category or blast-radius factor that determined risk_level.
- authorization_basis must briefly say whether authorization came from an explicit user request, the user's broader requested outcome, weak implication, or no relevant user evidence.
...
Keep every explanation short, generic, and on one line.`;
